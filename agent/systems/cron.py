"""
Cron scheduler — 5-field cron expressions, durable persistence, second-precision loop.

Cron jobs fire at matching minutes and are injected back into the agent loop
as "[Scheduled] <prompt>" messages.
"""
...
def _save_durable_jobs():
    ...
    DURABLE_CRON_PATH.write_text(json.dumps(durable, indent=2))
...
def _load_durable_jobs():
    if not DURABLE_CRON_PATH.exists():
        return
    try:
        for item in json.loads(DURABLE_CRON_PATH.read_text()):
            job = CronJob(**item)
